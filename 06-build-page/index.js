...
async function createHTML(template, components, assets, dst) {
  const templatePath = path.join(__dirname, template);
  const componentsPath = path.join(__dirname, components);
  const dstPath = path.join(__dirname, dst);
  const assetsPath = path.join(__dirname, assets);
  createTemplate(templatePath, componentsPath, dstPath);
  createCSS('styles', dst);
  copyDir(assetsPath, path.join(dstPath, assets));
}

async function createTemplate(templatePath, componentsPath, dstPath) {
...
async function createCSS(src, dst) {
...
createHTML('template.html', 'components', 'assets', 'project-dist');